...
def respond():
    ...
    if style == "clear" and topic:
        ...
        # End the session once enough words are taught — but not in the middle of
        # a question. If Morgan's reply ends with a question, let the student answer
        # first; the session will complete on the next turn.
        reached_target = len(taught_words) >= WORDS_PER_SESSION
