...
- Demonstrate each word with a real example and a reason, the way a friend would.
- Then draw the student in — invite them to talk about how they feel or what they think.
- Move at a natural pace — teach one thing, chat about it, then move to the next.
...
def get_chat_response(student_text: str, history: list, style: str = "casual",
    ...
    if style == "casual":
        ...
        teaching_context = (
            ...
            f"Teach the next word or phrase naturally by demonstrating it with a real "
            f"example and a reason, then invite the student to respond. Teach ONE new "
            f"thing at a time. Do not rush through several at once."
        )
...
- Wrong word choice where the meaning is affected (speak English fully → speak English fluently)
...
- Style preferences or informal but correct expressions
...
- Sentences that are slightly informal but still correct
...
If no errors: []"""
    ...
    user_prompt = (
        ...
        f"Find only clear genuine errors. When in doubt, return []. Return JSON array or []."
    )
